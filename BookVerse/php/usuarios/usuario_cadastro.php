...
&& isset($_REQUEST["email"]) && isset($_REQUEST["senha"])) {
    ...
    if($comando->fetch()) {
        $coisa = 1;
        $mensagem = "Usuário com RM existente! Digite outro!";
        echo json_encode(array("mensagem" => $mensagem, "coisa" => $coisa));
        exit();
    }
    $sql = "SELECT tb02_rm, tb02_nome, tb02_rm, tb02_senha, tb02_cod_veri
	FROM tb02_administradores where tb02_email = ?";
    $comando = $banco->prepare($sql);
    $comando->execute(array($email));
    if($comando->fetch()) {
        $coisa = 2;
        $mensagem = "Usuário com EMAIL existente! Digite outro!";
        echo json_encode(array("mensagem" => $mensagem, "coisa" => $coisa));
        exit();
    }
