...
if (
    ...
    isset($_FILES["foto"]) && isset($_FILES["pdf"])
) 
{
    $livro = $_POST["livro"];
    ...
    $nomeArquivo = $arquivo["name"];
        ...
        if (move_uploaded_file($foto["tmp_name"], "$dir/" . $nomeImagem) &&  move_uploaded_file($arquivoTemporario, $pdf)) {
            ...
            (NULL, ?, ?,
             ...
             (SELECT tb04_id_categoria FROM tb04_categoria WHERE tb04_nome_categoria = ?),
              ?, ?, ?,
              (SELECT tb05_id_periodo FROM tb05_periodo WHERE tb05_nome_periodo = ?), ?, NULL)";

            try {
                $comando = $banco->prepare($sql);
                $comando->execute(array($livro, $editora, $sinopse, 
                $autor, $categoria, $publicacao, 
                $nomeImagem, $nomePdf, $periodo,
                $classificacao));
